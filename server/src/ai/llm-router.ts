...
class ResponseCache {
  ...
  static hash(input: string): string {
    // 非加密用途，仅做缓存键；用简单滚动哈希避免引 crypto
    let h = 5381;
    for (let i = 0; i < input.length; i++) {
      h = ((h << 5) + h + input.charCodeAt(i)) | 0;
    }
    return `${input.length}:${h.toString(36)}`;
  }
}
...
export class LlmRouter {
  ...
  async chatJson<T>(tier: LlmTier, messages: ChatMessage[], opts?: { temperature?: number; maxTokens?: number }): Promise<{ data: T; usage: LlmUsage }> {
    ...
    // 修复重试：把模型自己的输出连同错误信息发回去
    logger.warn('LLM 返回非法 JSON，尝试修复', { tier, error: parsed.error });
    ...
    const res2 = await this.chat(tier, fixMessages, { ...opts, json: true });
