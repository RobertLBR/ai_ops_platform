...
- 📡 **实时监控开关**：Web 工作台一键开启 5s 级告警/诊断状态自动刷新，多 Tab 领导者选举去重，只读 DB 零 AI 调用
- ⚙️ **AI 服务配置生成**：粘贴日志样例即可让 AI 生成服务注册表条目，三层校验 + 备份写回 config.yaml + 热生效，全程可审计
...
| 🗂️ 服务注册表 | 注册服务、依赖关系、已知问题 + 数据源连通性；**AI 生成配置**入口与生成记录谱系 |
