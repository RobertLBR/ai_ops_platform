...
| B4 | 本环境实测约束：`diagnosis.levelFilter` 非空时 error（本环境 level 字段不可过滤，多数服务因无 log.log_level 字段而静默查 0 条日志）；datasource `maxDocs > 10000` 时 warning；`prometheusLabels.instance` 含反斜杠 → error | error/warning |
